...
[GENERATRON]
Filename:     controllersSF.js
Description:  Packaging Information
Project:      SFContacts

This file was generated from: ionic/controllersSF.js.vmg
...
.controller('OAuthCtrl', function ($scope, auth) {
  // save auth data
  $scope.onAuth = function (authData) {
    auth.set(authData);
  }
...
.controller('ContactsCtrl', function ($scope, ContactService,auth) {
	...
	var findAllContacts = function() {
		$.ajax({
			url:  authData.instance_url + '/services/data/v29.0/query/?q=SELECT+Name,Title+FROM+Contact',
			type: 'GET',
			headers: {
	  	...
	  	.then(function ok(e) {
	        ...
	        console.log("Result-->"+JSON.stringify(e.records) );
	    	$scope.contacts = e.records;
	    })
	  	}, function err(e) {
...
.controller('ContactCtrl', function ($scope, $stateParams, ContactService,auth) {
...
.controller('ContactSaveCtrl', function ($scope, $stateParams, ContactService,auth) {
    ContactService.save($stateParams.contactId).then(function(contact) {
        $scope.contact = contact;
    });
})

.controller('ContactUpdateCtrl', function ($scope, $stateParams, ContactService,auth) {
    ContactService.update($stateParams.contactId).then(function(contact) {
        $scope.contact = contact;
    });
})

.controller('ContactDeleteCtrl', function ($scope, $stateParams, ContactService,auth) {
